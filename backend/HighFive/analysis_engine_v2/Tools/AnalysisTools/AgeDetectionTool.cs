...
{
    public class AgeDetectionTool : AnalysisTool
    {
        private const string ModelPath = @"../../Models/vgg_ilsvrc_16_age_imdb_wiki.onnx";
        ...
        public override Data Process(Data data)
        {
            ...
            var age=((DenseTensor<float>) predictions.ElementAtOrDefault(0).Value).ToArray();
            
            return PostProcessFrame(data, age);
        }
        
        private Data PostProcessFrame(Data data, IReadOnlyList<float> ageWeights)
        {
            var output = new BoxCoordinateData();
            int age = 0;
            float weight = ageWeights[0];
            for (int i = 0; i < ageWeights.Count; i++)
            {
                if (ageWeights[i] > weight)
                {
                    age = i;
                    weight = ageWeights[i];
                }
